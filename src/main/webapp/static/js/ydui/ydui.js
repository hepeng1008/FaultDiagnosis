...
!function (window) {
    ...
    function OccupationSelect (element, options) {
        this.$element = $(element);
        this.options = $.extend({}, OccupationSelect.DEFAULTS, options || {});
        this.init();
    }

    OccupationSelect.DEFAULTS = {
        provance: '',
        city: '',
        area: ''
    };

    OccupationSelect.prototype.init = function () {
        var _this = this,
            options = _this.options;

        if (typeof YDUI_OCCUPATIONS == 'undefined') {
            console.error('请在ydui.js前引入ydui.occupation.js。');
            return;
        }

        _this.occupations = YDUI_OCCUPATIONS;
    ...
    OccupationSelect.prototype.open = function () {
        var _this = this;
        ...
        _this.$mask.on('click.ydui.occupationselect.mask', function () {
            _this.close();
        });
        ...
        $cityElement.find('.occupationselect-content').removeClass('occupationselect-move-animate occupationselect-next occupationselect-prev');
        ...
        if (defaultSet.provance) {
            ...
            $cityElement.find('.occupationselect-nav a').eq(0).addClass('crt').html('请选择');
        }
    ...
    OccupationSelect.prototype.close = function () {
        var _this = this;

        _this.$mask.remove();
        _this.$cityElement.removeClass('brouce-in').find('.occupationselect-nav a').removeClass('crt').html('');
        _this.$itemBox.html('');
    };

    OccupationSelect.prototype.createDOM = function () {
        var _this = this;

        _this.$mask = $('<div class="mask-black"></div>');

        _this.$cityElement = $('' +
            '<div class="m-occupationselect">' +
            '    <div class="occupationselect-header">' +
            '        <p class="occupationselect-title">所属职业</p>' +
            '        <div class="occupationselect-nav">' +
            '            <a href="javascript:;" ></a>' +
            '            <a href="javascript:;"></a>' +
            '            <a href="javascript:;"></a>' +
            '        </div>' +
            '    </div>' +
            '    <ul class="occupationselect-content">' +
            '        <li class="occupationselect-item">' +
            '            <div class="occupationselect-item-box"></div>' +
            '        </li>' +
            '        <li class="occupationselect-item">' +
            '            <div class="occupationselect-item-box"></div>' +
            '        </li>' +
        ...
        _this.$itemBox = _this.$cityElement.find('.occupationselect-item-box');

        _this.$cityElement.on('click.ydui.occupationselect', '.occupationselect-nav a', function () {
            var $this = $(this);
    ...
    OccupationSelect.prototype.setNavTxt = function (index, txt) {

        var $nav = this.$cityElement.find('.occupationselect-nav a');
    ...
    OccupationSelect.prototype.backOffView = function () {
        this.$cityElement.find('.occupationselect-content').removeClass('occupationselect-next')
            .addClass('occupationselect-move-animate occupationselect-prev');
    };

    OccupationSelect.prototype.ForwardView = function (animate) {
        this.$cityElement.find('.occupationselect-content').removeClass('occupationselect-move-animate occupationselect-prev')
            .addClass((animate ? 'occupationselect-move-animate' : '') + ' occupationselect-next');
    };

    OccupationSelect.prototype.bindItemEvent = function () {
        var _this = this,
            $cityElement = _this.$cityElement;

        $cityElement.on('click.ydui.occupationselect', '.occupationselect-item-box a', function () {
            var $this = $(this);
            ...
            var $nav = $cityElement.find('.occupationselect-nav a'),
                defaultSet = _this.defaultSet;

            if (tag == 0) {

                _this.loadCity();
                $cityElement.find('.occupationselect-item-box').eq(1).find('a').removeClass('crt');

            } else if (tag == 1) {

                _this.loadArea();
                _this.ForwardView(true);
                $cityElement.find('.occupationselect-item-box').eq(2).find('a').removeClass('crt');
    ...
    OccupationSelect.prototype.returnValue = function () {
        var _this = this,
            defaultSet = _this.defaultSet;

        _this.$element.trigger($.Event('done.ydui.occupationselect', {
            provance: defaultSet.provance,
            city: defaultSet.city,
            area: defaultSet.area
        }));
    ...
    OccupationSelect.prototype.scrollPosition = function (index) {
    ...
    OccupationSelect.prototype.fillItems = function (index, arr) {
        var _this = this;
    ...
    OccupationSelect.prototype.loadProvance = function () {
        var _this = this;

        var arr = [];
        $.each(_this.occupations, function (k, v) {
            arr.push($('<a class="' + (v.n == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;"><span>' + v.n + '</span></a>').data({
                occupations: v.c,
                tag: 0
            }));
    ...
    OccupationSelect.prototype.loadCity = function () {
        var _this = this;

        var cityData = _this.$itemBox.eq(0).find('a.crt').data('occupations');
    ...
    OccupationSelect.prototype.loadArea = function () {
        var _this = this;

        var areaData = _this.$itemBox.eq(1).find('a.crt').data('occupations');
        ...
        if (arr.length <= 0) {
            arr.push($('<a href="javascript:;"><span>全部</span></a>').data({tag: 2}));
        }
        _this.fillItems(2, arr);
    };
    ...
    function Plugin (option) {
        ...
        return this.each(function () {
            var $this = $(this),
                occupationSelect = $this.data('ydui.occupationselect');

            if (!occupationSelect) {
                $this.data('ydui.occupationselect', (occupationSelect = new OccupationSelect(this, option)));
            }

            if (typeof option == 'string') {
                occupationSelect[option] && occupationSelect[option].apply(occupationSelect, args);
            }
        });
    }

    $.fn.occupationSelect = Plugin;
    ...
    InfiniteScroll.prototype.loadList = function () {
        ...
        if (typeof options.loadListFn == 'function') {
            ...
            options.loadListFn().done(function (listArr, page) {
                ...
                // 此次请求的数据等于pageSize[每页请求数据数]，且未能填充满页面，则继续请求下一页数据
                if(len == options.pageSize && _this.$element[0].scrollHeight <= _this.$element[0].offsetHeight){
                    _this.loadList();
                }
            });
        }
